...
export async function searchVideos(query: string): Promise<SearchResponse> {
	try {
		const response = await fetch(
			`${API_BASE_URL}/videos/search?query=${encodeURIComponent(query)}`
		);
		...
		relatedContent: [
			{
				...
				snippet:
					'Brief description of related content that might interest you based on your search.'
			},
			{
				id: 'related-2',
				title: 'Related Topic B',
				snippet:
					'Another piece of related content that provides additional context to your search query.'
			},
			{
				id: 'related-3',
