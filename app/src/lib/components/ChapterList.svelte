	...
	<div class="flex items-center justify-between">
		...
		{#if chapters.length > 0}
			<span class="text-xs text-gray-500"
				>{chapters.length} chapter{chapters.length !== 1 ? 's' : ''}</span
			>
		{/if}
	</div>

	<!-- Scrollable chapters list with max height -->
	<div class="max-h-96 space-y-3 overflow-y-auto pr-2" bind:this={scrollContainer}>
		{#each chapters as chapter, index}
			<div
				class={`relative cursor-pointer rounded-lg border p-3 transition
          ${
						activeChapter === chapter.title
							? 'border-primary-400 bg-primary-50 shadow-sm'
							: chapter.isRelevant
								? 'border-secondary-300 bg-secondary-50 shadow-sm hover:bg-secondary-100'
        ...
        `}
			...
			>
				...
				<div class="mb-1 flex items-start justify-between">
					<p class="pr-16 font-medium text-gray-800">{chapter.title}</p>
